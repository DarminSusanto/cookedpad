...
for ($i = 1; $i -le 1000; $i++) {
    Invoke-WebRequest http://cookedpad.local/health -ErrorAction SilentlyContinue &
}
Wait-Job

# Atau pakai Apache Bench (cross-platform):
ab -n 10000 -c 100 http://cookedpad.local/
```

**Segment 4: Show Scaling Events (1 menit)**
```powershell
kubectl describe hpa cookedpad-frontend -n cookedpad | grep -A 10 "Events:"
